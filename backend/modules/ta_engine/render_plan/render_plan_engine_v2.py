...
class RenderPlanEngineV2:
    ...
    def _build_patterns_layer(
        ...
        # Filter valid patterns (exclude channels which should be filtered by PatternSelector)
        valid_patterns = []
        for p in patterns:
            # Double-check: skip any channel/trend that might have slipped through
            p_type = (p.get("type") or "").lower()
            if any(kw in p_type for kw in ["channel", "trend", "range", "sideways"]):
                continue
            
            # Check score and state
            if p.get("score", 0) >= 0.5 and p.get("state") in ["forming", "active"]:
    ...
    def _format_pattern(self, pattern: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "bias": pattern.get("bias", pattern.get("direction_bias")),
